...
class HgeClient:
    ...
    def __get_url(self, service_name, uri):
        if self.__local_cluster_mode:
            return "http://{host}/{uri}".format(host=self.__service_url_dict.get(self.__service_tag, {}).get(service_name), uri=uri.lstrip("/"))
        else:
            return "http://{vip}:11000/{tag}/v1.0/{service}/{uri}".format(vip=self.__host, tag=self.__service_tag, service=service_name, uri=uri.lstrip("/"))
