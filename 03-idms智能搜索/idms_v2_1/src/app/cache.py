...
2ndkey  :
    "startLabel":
    "endLabel":
    "data":{key---in(right) data, --->(this data)
            value-out(left)  data,(this data)--->
            }
'''
g_relation_dict = {
    "rFeatureaboutConfig":{
        "startLabel":"iFeatureV7B70",
        "endLabel":"iConfig",
        "data":{}},
    "rFeatureaboutWord":{
        "startLabel":"iFeatureV7B70",
...
def req(keyList):
    ...
    for k in list(keyList):
        ...
        '''
        "data": [
                    "{name=[交换机产品演示分支], alias=[iVerSwitchBranchDemo]}",
                    "{name=[交换机产品irf3分支], alias=[iVerSwitchBranchIRF3]}",
                    "{name=[交换机产品jr分支], alias=[iVerSwitchBranchJR]}",
                    "{name=[交换机产品arad分支], alias=[iVerSwitchBranchARAD]}"
                ],
        '''
