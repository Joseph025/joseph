...
with vertica_python.connect(**vertica_config) as connection:
    ...
    cur.execute("""select * from E2E.wufan_book""")
    ...
    with driver.session() as session:
        ...
        for row in cur.iterate():
            ...
            # 当没有label的时候创建，有就新加
            # tx.run('merge (Book:' + obj['label'] + '{kemu:$kemu, kemu_id:$kemu_id})',**obj)

            # 当没有label的时候新建label，有label且如果已存数据有更新就更新，有新增数据就新加。
            # 但是当数据量大的时候会影响性能
            tx.run('merge (n:' + obj['label'] + '{kemu_id:$kemu_id}) set n+= {kemu:$kemu}', **obj)
        ...
        if count> 0:
            tx.commit()
    driver.close()
logging.info('create node 书 seccess')